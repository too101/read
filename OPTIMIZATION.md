...
|---|---:|---:|---:|
| ขนาดไบนารี (`read.com`) | 15,349 B | **7,679 B** | **เล็กลง 50.0% (2.00×)** |
...
### 5.13 `exit_help` ไม่คำนวณขีดจำกัดเลื่อนขวา (`maxh`) ใหม่ ทำให้ไฟล์กว้างเสียการเลื่อนขวาหลังเข้า-ออกหน้า help *(พบระหว่างพอร์ตโปรแกรมไปวินโดวส์ เทียบ logic กับต้นฉบับทีละบรรทัด)*

`exit_help` (ตอนกด F1 ออกจากหน้า help กลับไปไฟล์เดิม) restore ค่า
`top`/`hshift`/`nlines`/`maxlen`/`ku_mode` จากที่เก็บไว้ตอนเข้า help แล้วเรียก
แค่ `calc_top` (คำนวณ `topmax` ใหม่จาก `nlines` ที่เพิ่งกลับมา) ไม่ได้เรียก
ส่วนคำนวณ `maxh` ใหม่เลย ต่างจากตอนเข้า help (ข้อ 5.10) ที่เรียกครบทั้งคู่ผ่าน
`calc_limits`

**ผลคือ:** เปิดไฟล์กว้างเกิน 80 คอลัมน์ (`maxh>0`) แล้วกด F1 ดู help (help กว้าง
พอดี 80 คอลัมน์ จึงคำนวณ `maxh=0` ตอนเข้า) แล้วกด F1 ออกมา — `maxh` ของไฟล์ค้าง
อยู่ที่ 0 ของ help แทนที่จะกลับไปเป็นค่าจริงของไฟล์ เลื่อนขวาดูเนื้อหาที่ตกขอบ
ไม่ได้อีกต่อไปทั้งที่ไฟล์ยังกว้างเกิน 80 คอลัมน์อยู่จริง (`top`/`hshift`/`topmax`
ยัง restore ถูกต้องปกติ มีแค่ `maxh` จุดเดียวที่ตกหล่น)

**วิธีแก้:** แยกส่วนคำนวณ `maxh` ออกจาก `calc_limits` เป็น routine ย่อย
`calc_maxh` (แล้วให้ `calc_limits` เรียกต่อแบบ tail call เหมือนเดิมทุกจุด ไม่มี
ผลต่อพฤติกรรมเดิมของ `calc_limits`) จากนั้นเพิ่มการเรียก `calc_maxh` ใน
`exit_help` ต่อจาก `calc_top` — ตั้งใจไม่เรียก `calc_limits` ตรง ๆ เพราะ
`calc_limits` รีเซ็ต `top`/`hshift` กลับ 0 เสมอ ซึ่งจะทำลายตำแหน่งเลื่อนจอที่
เพิ่ง restore มาก่อนเข้า help

**ตรวจสอบแล้ว:** เปรียบเทียบกับ binary ก่อนแก้ (ย้อนแค่บรรทัดที่เพิ่ม ส่วนอื่น
เหมือนกันทุกไบต์) ผ่าน emulator — สถานการณ์ "เปิดไฟล์กว้าง เลื่อนขวา N ครั้ง"
กับ "เปิดไฟล์เดียวกัน กด F1 เข้า-ออก แล้วเลื่อนขวา N ครั้งเท่ากัน" ให้ภาพจอตรงกัน
เป๊ะหลังแก้ (ต่างกันก่อนแก้ ยืนยันบั๊กมีจริงและแก้ตรงจุด) และรัน regression
suite มาตรฐานเดิมซ้ำ (ทุก scenario เดิม ทุกโหมดจอ) แล้วไม่มีจุดต่างจากก่อนแก้
แม้แต่พิกเซลเดียว (สมเหตุสมผล เพราะ suite เดิมไม่เคยเลื่อนขวาหลัง F1/F1 ใน
สคริปต์ทดสอบ จึงไม่เคยไปแตะโค้ดที่เพิ่งแก้)

---

## 6. การใช้หน่วยความจำ

- **บนดิสก์:** 7,679 ไบต์ (จากเดิม 15,349)
