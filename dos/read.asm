...
exit_help:
        ...
        call    set_ku
        call    calc_top
        call    calc_maxh       ; refresh maxh for the restored file (it was
                                 ; left at help's own maxh=0 otherwise, which
                                 ; would wrongly disable right-arrow scroll on
                                 ; a file wider than the screen after F1/F1)
        mov     word [lin_base], lin_tab
        mov     word [lin_lim], LIN_LIM
        ret
...
; calc_maxh: maxh = max(maxlen-text_cols, 0). text_cols is mode-specific
; (80 for CGA/EGA/VGA, 90 for Hercules -- see vparm) so the scroll threshold
; matches what draw_line actually fits on screen in the active mode, not a
; fixed 80 that would falsely enable scroll on HGC for lines 81-90 columns
; wide (which fit HGC fine).
calc_maxh:
        mov     ax, [maxlen]
        mov     bl, [text_cols]
        xor     bh, bh
...
calc_limits:
        ...
        mov     word [top], 0
        mov     word [hshift], 0
        jmp     calc_maxh       ; tail call: calc_maxh's ret returns to caller
