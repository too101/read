...
(อัปเดตล่าสุด: **binary 7,679 ไบต์ (เดิม ~15.3K, เล็กลง 50.0%), source ~1,660 บรรทัด** — หลังรอบ optimize + แก้บั๊กเลื่อนขวา 5 จุด)

> **สรุปรอบ optimize:** เล็กลง **50.0%** (15,349 → 7,679 ไบต์), เร็วขึ้น **~2.4–2.7× ตอนเปิด** และ
> **~2.3–4.7× ต่อการกดปุ่ม** (บน 8088 ก็เร็วขึ้นใกล้เคียงกัน — ดู §11), พิกเซลตรงกับ reference ที่แก้บั๊กแล้ว
> 100% บนไฟล์จริง (ยกเว้น HGC บรรทัดกว้าง 81+ คอลัมน์ — ดู ★19) และ **แก้บั๊ก status bar เพี้ยนตอนเลื่อน**
> (ชื่อไฟล์ซ้อน) กับบั๊กแฝงอีก 8 จุด รวมถึง 5 จุดล่าสุดที่ทำให้บรรทัดยังเลื่อนขวาได้ทั้งที่ไม่ควร (ดู ★16–★20 ใน §12)
...
- **จำแนกไบต์จาก `cls[]`** (ตารางเดียวกับ render) — mark/กลืน/style ไม่เพิ่มคอลัมน์ (กว้าง 0 เหมือนตอน `draw_line` จริง — เดิมเคยนับ style เป็นคอลัมน์ตาม TREAD จนเป็นบั๊ก ★16 ดู §12), CR/LF/CRLF = ตัวแบ่ง, จำกัด 255 คอลัมน์/บรรทัด (`dl`), รีเซ็ต `maxlen=0` ทุกครั้งที่เริ่มสแกนตารางใหม่ (★18)
...
- **enter_help**: save top/hshift/nlines/maxlen/ku (5 words) + set `blk0=CS` → `lin_base=help_lin_tab` → **build help แบบ lazy ครั้งเดียว** (`help_built`, จำ `help_nlines`) → เรียก `calc_limits` (ไม่ใช่แค่ `calc_top`) เพื่อ recompute ทั้ง `topmax` **และ** `maxh` จาก `maxlen` ของ help เอง แล้ว top=0 — ตารางของไฟล์**ไม่ถูกแตะ** (เดิมเรียกแค่ `calc_top` ทำให้ help สืบทอด `maxh` ของไฟล์ก่อนหน้าค้างมา — บั๊ก ★17 ดู §12)
...
วัดด้วย emulator 8086 แบบ instruction-accurate (Unicorn) + โมเดล cycle ตามตาราง Intel 8086 (EA cost, branch taken/not-taken, string ops ต่อรอบ) เทียบ `read_orig.com` (15,349 ไบต์) กับฉบับ optimize (7,679 ไบต์):

| Metric | เดิม | ใหม่ | ดีขึ้น |
|---|---:|---:|---:|
| ขนาดไฟล์ | 15,349 B | **7,679 B** | **50.0% (2.00×)** |
...
จับด้วย emulator harness + pixel-diff (§13) แก้แล้วทั้งหมด — แยกเป็นบั๊กเดิมที่พบตอน RE/optimize และบั๊กที่แก้ในรอบ optimize (★19 เป็นข้อยกเว้นเดียว: แก้แล้วแต่ตั้งใจให้ต่างจาก reference บน HGC — ดูรายละเอียดในแถวนั้น):
...
| ★16 | **บรรทัดพอดี 80 คอลัมน์แต่ยังเลื่อนขวาได้** (พบตอนใส่ `HELP.TXT` ที่ตั้งใจทำให้พอดี 80 คอลัมน์ ซึ่งมีบรรทัด legend โชว์ตัวอย่างสไตล์ทุกแบบแทรกไบต์สลับสไตล์ 14 ตัวกลางบรรทัด) | `build_lines` นับไบต์สลับสไตล์ (C_STYLE) เป็น **1 คอลัมน์ต่อไบต์** ตามพฤติกรรมเดิมของ TREAD แต่ `draw_char_c`/`style_toggle` ไม่เคยขยับ `cur_col` ตอนเจอไบต์พวกนี้จริง (กว้าง 0 บนจอ) → บรรทัดเห็น 80 คอลัมน์ถูกนับผิดเป็น 94 → `maxh>0` ทั้งที่ไม่ควร | ตัวนับความกว้างสำหรับ scroll ต้องนับตรงกับสิ่งที่ตัววาดจอ*จริง*ทำ ไม่ใช่ตามพฤติกรรมของโปรแกรมอื่นที่อ้างอิงไว้ตอนแรก — แก้ให้ C_STYLE ตกไปอยู่กลุ่มกว้าง-0 เหมือน C_SWAL/C_COMB |
| ★17 | **หน้า help สืบทอดขีดจำกัดเลื่อนขวาจากไฟล์ที่เปิดค้างไว้ก่อนหน้า** (พบระหว่างตรวจ ★16) | `enter_help` เรียกแค่ `calc_top` (คำนวณ `topmax` อย่างเดียว) ไม่ได้เรียก `calc_limits` จึง `maxh` ยังเป็นค่าจากไฟล์ก่อนหน้า ไม่ใช่ของ help เอง | ทุก entry point ที่สลับไปใช้ "ตารางบรรทัด" ชุดใหม่ ต้อง recompute ขีดจำกัดที่ผูกกับตารางนั้น**ทั้งหมด** ไม่ใช่แค่บางส่วน — แก้ให้เรียก `calc_limits` แทน |
| ★18 | **`maxlen` ไม่เคยรีเซ็ต ค้างสูงข้ามการเปิดไฟล์/help ครั้งใหม่** (พบระหว่างตรวจ ★16–★17 ด้วยไฟล์ทดสอบกว้าง 120 คอลัมน์) | `build_lines` อัปเดต `[maxlen]` แบบมีแต่เพิ่มขึ้น (`cmp`/`jbe`) ไม่เคยรีเซ็ตกลับ 0 ตอนเริ่มสแกนตารางใหม่ — มีค่าจาก BSS zero-init ครั้งเดียวตอนโปรแกรมเริ่ม | ตัวแปร "high-water mark" ที่คำนวณใหม่ทุกครั้งที่ scan ต้องรีเซ็ตตอนเริ่ม scan ทุกรอบ ไม่ใช่แค่ตอนโปรแกรมเริ่ม — แก้ให้ `build_lines` ตั้ง `[maxlen]=0` ที่จุดเริ่มฟังก์ชัน |
| ★19 | **ขีดจำกัดเลื่อนขวา hardcode 80 ทุกโหมด ทั้งที่ HGC แสดงได้ 90 คอลัมน์จริง** (ผู้ใช้ถามหลังเห็น ★16–★18: "ไม่กระทบ HGC ที่เป็น 90 column ใช่ไหม") — **มีมาตั้งแต่ต้นฉบับก่อน optimize** ไม่ใช่บั๊กที่เพิ่งเกิดจาก ★16–★18 | `vparm` กำหนด `text_cols` ของ HGC ไว้ 90 (720÷8) และ `draw_line` ก็ใช้ 90 จริงตอนวาด แต่ `calc_limits` (ทั้งใน `read_orig.com` และ `read_ref.com`) ลบด้วยเลข `80` คงที่เสมอ ไม่อ้างอิง `[text_cols]` ของโหมดที่ใช้งานอยู่ | ค่าคงที่ที่ "ดูเหมือน" ใช้ได้ทุกโหมดต้องเช็คว่าจริงหรือแค่บังเอิญตรงกับโหมดที่ทดสอบบ่อยที่สุด (CGA/EGA/VGA=80 ทำให้ไม่มีใครสังเกตว่า HGC=90 ต่างออกไป) — แก้ให้ `calc_limits` ลบด้วย `[text_cols]` แทน 80 คงที่ (`mov bl,[text_cols] / xor bh,bh / sub ax,bx`) **จุดนี้เป็นจุดเดียวในรายการนี้ที่ตั้งใจให้ผลต่างจาก `read_ref.com`** — เฉพาะโหมด HGC บรรทัดกว้าง 81+ คอลัมน์เท่านั้น เพราะ reference เองก็มีบั๊กเดิมติดมา (ตรวจแล้วด้วย emulator: กวาดความกว้าง 70–100 คอลัมน์ทั้ง 4 โหมด, CGA/EGA/VGA ไม่ต่างจาก reference สักจุด, HGC เริ่มต่างที่ 81 คอลัมน์พอดีตามที่ตั้งใจ) |
| ★20 | **`exit_help` ไม่คำนวณ `maxh` ใหม่ — ไฟล์กว้างเสียการเลื่อนขวาหลังเข้า-ออกหน้า help** (พบระหว่างพอร์ตโปรแกรมไปวินโดวส์ ตอนเทียบ logic `exit_help` กับต้นฉบับทีละบรรทัด) | `exit_help` restore `top`/`hshift`/`nlines`/`maxlen`/`ku_mode` จากค่าที่บันทึกไว้ตอนเข้า help แล้วเรียกแค่ `calc_top` (คำนวณ `topmax` ใหม่) — ไม่เรียกส่วนคำนวณ `maxh` เหมือนตอนเข้า help (★17) ผลคือถ้าเปิดไฟล์กว้าง >80 คอลัมน์ แล้ว F1 เข้า help (กว้างพอดี 80 → `maxh=0`) แล้ว F1 ออก `maxh` ของไฟล์ค้างที่ 0 ของ help แทนค่าจริง เลื่อนขวาไม่ได้อีกต่อไปทั้งที่ไฟล์ยังกว้างเกิน 80 คอลัมน์จริง | เหมือน ★17 แต่กลับทิศ: entry point ที่สลับ "กลับ" ไปตารางบรรทัดชุดเดิมก็ต้อง recompute ขีดจำกัดที่ผูกกับตารางนั้นให้ครบเช่นกัน ไม่ใช่แค่ตอนสลับเข้า — แก้โดยแยก `calc_maxh` ออกจาก `calc_limits` เป็น routine เดี่ยว แล้วให้ `exit_help` เรียกต่อจาก `calc_top` โดยตั้งใจ**ไม่**เรียก `calc_limits` ตรง ๆ เพราะจะรีเซ็ต `top`/`hshift` ที่เพิ่ง restore กลับ 0 ซึ่งผิด — ยืนยันด้วย emulator: เทียบภาพจอ "เปิดไฟล์กว้าง เลื่อนขวา N ครั้ง" กับ "เปิดไฟล์เดียวกัน F1 เข้า-ออก แล้วเลื่อนขวา N ครั้ง" ตรงกันเป๊ะหลังแก้ (ต่างกันก่อนแก้) และรัน regression suite เดิมซ้ำไม่มีผลข้างเคียง (suite เดิมไม่เคยเลื่อนขวาหลัง F1/F1 จึงไม่เคยแตะจุดนี้มาก่อน) |
...
| `read.com` | binary สุดท้าย (**7,679 ไบต์**) |
