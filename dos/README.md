...
ต้นฉบับดั้งเดิมของโปรเจกต์นี้ — พอร์ตต่อยอดไปวินโดวส์ ([`windows/`](../windows),
x86 asm ล้วน) และ Linux ([`linux/`](../linux), C + X11) ดูภาพรวมทั้งโปรเจกต์ที่
[README หลัก](../README.md) และรายงานผล optimize แบบละเอียดที่
...
| `output/read.com` | ผลลัพธ์จาก build (ไม่ commit เข้า git — สร้างใหม่ได้เสมอด้วย `python build_read.py`) |
| `READ_TECHNICAL.md` | เอกสารเทคนิคฉบับสมบูรณ์ — สถาปัตยกรรม, memory map, ระบบทดสอบ, บั๊กที่พบทั้งหมด |
| `STORY.md` | ที่มาของโปรเจกต์ (เรื่องเล่า) |
...
|---|---:|---:|---:|
| ขนาดไบนารี | 15,349 B | 7,679 B | **เล็กลง 50.0% (2.00×)** |
...
12. **ขีดจำกัดเลื่อนขวา hardcode ไว้ที่ 80 คอลัมน์ทุกโหมด ทั้งที่ Hercules
    ...
    นั้นคือ 80 อยู่แล้ว
13. **ออกจากหน้า help แล้วเลื่อนขวาดูไฟล์กว้างไม่ได้อีกต่อไป** (พบระหว่างพอร์ต
    โปรแกรมไปวินโดวส์ แล้วเทียบ logic กับต้นฉบับทีละบรรทัด) `exit_help` (ตอนกด
    F1 ออกจากหน้า help) เรียกแค่ routine คำนวณจำนวนบรรทัดใหม่ (`calc_top`)
    ไม่ได้เรียก routine คำนวณขีดจำกัดเลื่อนขวาใหม่ (`maxh`) ด้วย เหมือนตอนเข้า
    help (ข้อ 10 ด้านบน) ผลคือถ้าเปิดไฟล์กว้างเกิน 80 คอลัมน์ แล้วกด F1 เข้าไป
    ดู help ครั้งหนึ่ง (ซึ่งกว้างพอดี 80 คอลัมน์ จึง `maxh=0`) แล้วกด F1 ออกมา —
    `maxh` ของไฟล์จะค้างอยู่ที่ 0 ของ help แทนที่จะกลับไปเป็นค่าจริงของไฟล์ เลื่อน
    ขวาดูเนื้อหาที่ตกขอบไม่ได้อีกต่อไปทั้งที่ไฟล์ยังกว้างเกิน 80 คอลัมน์อยู่จริง
    แก้โดยแยกส่วนคำนวณ `maxh` ออกมาเป็น routine ย่อย (`calc_maxh`) แล้วเรียกซ้ำ
    จาก `exit_help` ด้วย โดยไม่แตะ `top`/`hshift` ที่เพิ่ง restore มาจากก่อนเข้า
    help (ต่างจาก `calc_limits` ที่รีเซ็ต `top`/`hshift` กลับ 0 เสมอ ซึ่งใช้ไม่ได้
    ในจุดนี้) ยืนยันด้วย emulator แล้วว่าแก้ตรงจุด (เปิดไฟล์กว้าง เลื่อนขวา แล้ว
    เทียบกับ: เปิดไฟล์เดียวกัน กด F1 เข้า-ออก แล้วเลื่อนขวาเท่ากัน — ภาพตรงกันเป๊ะ
    หลังแก้ ต่างกันก่อนแก้) และรัน regression suite เดิมซ้ำแล้วไม่มีผลข้างเคียงอื่น
14. **ไฟล์รหัส KU เลื่อนแนวนอนได้ไกลเกินจริง** (พบจากผู้ใช้ทดสอบไฟล์รหัส KU
    จริง: DOS เลื่อนได้ถึง `C:15` ทั้งที่เนื้อหาที่เห็นบนจอกว้างสุดแค่ 81
    คอลัมน์ ควรเลื่อนได้แค่ `C:1`) ต้นเหตุคือ `build_lines` (`bl_c`) จัด
    ประเภทไบต์ **ดิบ** ตรงจากไฟล์ผ่านตาราง `cls` เลย ไม่ผ่านการแปลงรหัส
    KU→TIS ก่อน ต่างจากตอนวาดข้อความจริงที่ใช้ตาราง `trc` (สร้างโดย
    `set_ku`: `trc[b] = class<<8 | translate(b)`) ซึ่งแปลงรหัสก่อนจัด
    ประเภทเสมอ — ไบต์สระ/วรรณยุกต์แบบ KU ดิบมักไม่ตกในช่วงไบต์ combining
    mark ของ TIS-620 (`D1h`-`EEh`) เลยถูกนับเป็น 1 คอลัมน์เต็มแทนที่จะเป็น 0
    เหมือนตอนวาดจริง (ซ้อนบนตัวฐาน ไม่กินพื้นที่) ทำให้ `maxlen`/`maxh` สูง
    เกินจริงเฉพาะไฟล์รหัส KU เท่านั้น แก้โดยให้ `bl_c` ใช้ตาราง `trc` ตัว
    เดียวกับ `RDCH` แทนตาราง `cls` ดิบ (`AH`=คลาส มาจาก `trc`, ไม่ใช่
    `cls` โดยตรงอีกต่อไป — `AL` หลังจากนี้เป็นไบต์ที่แปลงรหัสแล้ว แต่ไม่กระทบ
    การเทียบ `0Dh`/`0Ah` ใน `bl_term` เพราะไบต์ต่ำกว่า `80h` ไม่เคยถูกแปลง)
    และแก้ให้ปุ่ม `c` (สลับ KU/TIS ระหว่างดูไฟล์) เรียก `build_lines` +
    `calc_maxh` ใหม่ทุกครั้งด้วย (เดิมสลับโหมดแล้วไม่รีเฟรชค่านี้เลย ซึ่งไม่
    เคยเป็นปัญหาตอนนับจากไบต์ดิบเพราะโหมด KU ไม่มีผลต่อการนับ แต่พอนับผ่าน
    `trc` แล้วโหมด KU มีผลโดยตรง) พร้อม clamp `hshift` ไม่ให้เกิน `maxh`
    ใหม่ ยืนยันด้วยการรันโค้ดที่ประกอบแล้วจริงผ่าน emulator (Unicorn) ป้อน
    ไฟล์ทดสอบจริงของผู้ใช้เข้า `detect_ku`/`build_lines` ตรง ๆ: ก่อนแก้ได้
    `maxlen=95 maxh=15` (`C:15`, ตรงกับที่ผู้ใช้รายงาน) หลังแก้ได้
    `maxlen=81 maxh=1` (`C:1`) — ตรงกับพอร์ต [Windows](../windows) และ
    [Linux](../linux) เป๊ะ (ทั้งสองพอร์ตนับแบบแปลงรหัสก่อนนับมาตั้งแต่แรก
    อยู่แล้ว จึงไม่ต้องแก้ ยกเว้นแก้ปุ่ม `c` ให้ rebuild เหมือนกัน) ไฟล์ที่
    ไม่ใช่รหัส KU ไม่กระทบเลย (ทดสอบแล้วได้ `maxlen`/`maxh` เท่าเดิมทั้งก่อน
    และหลังแก้)
